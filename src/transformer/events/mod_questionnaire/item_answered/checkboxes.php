...
/**
 * This function creates answer log about checkbox item.
 ...
 */
function checkboxes(array $config, \stdClass $event, \stdClass $question, $tablename, $responseid, \stdClass $questionnaire) {
    $repo = $config['repo'];
    ...
    foreach ($records as $record) {
        ...
        if (!empty($itemrecord)) {
            array_push($tmparray, '{' . $itemrecord->content . '}');
        }
    }
    ...
    return [[
        ...
        'result' => [
            ...
            "extensions" => [
                "http://learninglocker.net/xapi/cmi/choice/response" => $choices,
            ],
        ],
        'context' => [
