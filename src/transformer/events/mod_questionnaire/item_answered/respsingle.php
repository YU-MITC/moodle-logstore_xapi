...
/**
 * This function creates answer log about respsingle item.
 ...
 */
function respsingle(array $config, \stdClass $event, \stdClass $question, $tablename, $responseid, \stdClass $questionnaire) {
    $repo = $config['repo'];
    ...
    if (!empty($choice)) {
        $selectchoice = $repo->read_record_by_id('questionnaire_quest_choice', $choice[0]->choice_id);
        $choicelabel = $selectchoice->content;
    }
        ...
        'context' => [
            ...
            'contextActivities' => [
                'grouping' => [
                    ...
                    utils\get_activity\course_questionnaire($config, $course, $event->contextinstanceid),
                ],
                'category' => [
